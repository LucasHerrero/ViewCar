...
@Component({
  ...
  template: `
    <div class="p-5 px-7 rounded-md text-center">
      ...
      <h1 class="font-bold text-lg mb-2">{{ data.title }}</h1>
      <p>{{ data.message }}</p>
      <button *ngIf="data.status === 'password'"
        class="mt-4 bg-black w-full text-white py-2 rounded-md"
        (click)="error()"
      >
        Aceptar
      </button>
      <button *ngIf="data.status === 'success'"
        class="mt-4 bg-black w-full text-white py-2 rounded-md"
        (click)="login()"
      >
        Iniciar sesión
